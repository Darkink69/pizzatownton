...
function Tasks() {
  ...
  // Эффект для adsgram-task (отправка TASKS_COMPLETE ADS_TASK_1)
  useEffect(() => {
    const handler = () => {
      ...
      localStorage.setItem("adsTaskDone", "true");
    };
  ...
  useEffect(() => {
    ...
    const adsDone = localStorage.getItem("adsTaskDone") === "true";
  ...
  // Рендерим adsgram-task только если он загружен и ещё не выполнен
