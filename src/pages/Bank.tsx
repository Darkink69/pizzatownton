                            ...
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setDetailOpen(true);
                                    store.requestAdminWithdrawDetail(item.id, item.telegramId);
                                  }}
                                  className="px-2 py-1 rounded text-[11px] font-bold bg-amber-200 hover:bg-amber-300 text-amber-900"
                          ...
                          <td className="border border-amber-300 px-3 py-2">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation(); // ✅ не открывать details при копировании
                                    handleCopyAddress(walletAddress);
                                  }}
                                  className={`p-1 rounded ${
                                      isCopied ? "text-green-600" : "text-gray-400 hover:text-amber-600"
                                  }`}
      ...
      {translateStatus(item.status)}
                          ...
                          <td className="border border-amber-300 px-3 py-2">
                            <div className="flex items-center gap-2">
                              <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation(); // ✅ не открывать details при approve
                                    store.adminApprove(item.id);
                                  }}
                              ...
                              <button
                                  ...
                                  disabled={item.status !== "PENDING"}
                                  onClick={(e) => {
                                    e.stopPropagation(); // ✅ не открывать details при reject
                                    store.adminReject(item.id);
                                  }}
                                  className={`w-8 h-8 flex items-center justify-center rounded font-bold text-white ${
                                      item.status === "PENDING"
              ...
              {detailOpen && (
                  <div className="fixed inset-0 z-[90] bg-black bg-opacity-70 flex items-center justify-center p-4">
                    <div
                        className="bg-white rounded-lg p-6 max-w-xl w-full shadow-lg border-2 border-amber-800 shantell text-amber-800 relative">
                      <button
                          onClick={() => {
                            setDetailOpen(false);
                            store.setAdminDetail(null);        // очистить данные
                            store.setAdminDetailError("");     // опционально
                          }}
                          className="absolute -top-10 right-2 w-8 h-8 bg-transparent hover:scale-110 transition-transform"
                      >
                        <img
                            src={`${store.imgUrl}b_close.png`}
                            alt="Закрыть"
                      ...
                      {store.adminDetailLoading && (
                          <div className="text-sm text-amber-700">Загрузка...</div>
                      )}

                      {!store.adminDetailLoading && store.adminDetailError && (
                          <div className="text-sm text-red-600">{store.adminDetailError}</div>
                      )}

                      {!store.adminDetailLoading && store.adminDetail && (
                          <div className="text-sm space-y-2">
                            <div><b>TG:</b> {store.adminDetail.telegramId}</div>
                            <div>
                              <b>Адрес:</b>{" "}
                              <span className="font-mono break-all">{store.adminDetail.tonAddress}</span>
                            </div>
                            <div><b>TON:</b> {Number(store.adminDetail.amount ?? 0).toFixed(8)}</div>
                            <div><b>Создано:</b> {store.adminDetail.createdAt}</div>

                            <hr className="border-amber-200 my-3"/>

                            <div><b>Купил PCoin (раз):</b> {store.adminDetail.buyPcoinCount}</div>
                            <div><b>Внёс TON всего:</b> {Number(store.adminDetail.buyPcoinTonTotal ?? 0).toFixed(4)}
                            </div>
                            <div><b>Получил PCoin всего:</b> {store.adminDetail.buyPcoinPcoinTotal}</div>

                            <hr className="border-amber-200 my-3"/>

                            <div>
                              <b>Этажи:</b> {store.adminDetail.openedTotalFloors} (обычных: {store.adminDetail.openedRegularFloors})
                            </div>
                            <div><b>Фарм PD/час:</b> {Number(store.adminDetail.pdollarPerHourTotal ?? 0).toFixed(2)}
                            </div>
                            <div><b>Фарм Pizza/час:</b> {Number(store.adminDetail.pizzaPerHourTotal ?? 0).toFixed(2)}
                            </div>

                            <hr className="border-amber-200 my-3"/>

                            <div><b>Total income TON:</b> {Number(store.adminDetail.totalIncome ?? 0).toFixed(4)}</div>
                            <div><b>Withdraw COMPLETED
                              TON:</b> {Number(store.adminDetail.totalWithdraws ?? 0).toFixed(4)}</div>

                            {store.adminDetail.isBlogger && (
                                <>
                                  <hr className="border-amber-200 my-3"/>
                                  <div><b>Ref count:</b> {store.adminDetail.refCount}</div>
                                  <div><b>Ref income TON:</b> {Number(store.adminDetail.refIncomeTon ?? 0).toFixed(4)}
                                  </div>
                                  <div><b>Ref withdraw
                                    TON:</b> {Number(store.adminDetail.refWithdrawsTon ?? 0).toFixed(4)}</div>
                                </>
                            )}
                          </div>
                      )}
                    ...
                    <div className="relative flex flex-col p-5">
                      ...
                      <div className="relative z-10 flex flex-col">
                        ...
                        <CurrencyInput
                            ...
                            min={1}
                            formatValue={(value: { toString: () => string }) => {
                              // Форматируем значение с пробелами через каждые 3 цифры
                              return value
                                  .toString()
                                  .replace(/\B(?=(\d{3})+(?!\d))/g, " ");
                            }}
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <div
                                className="bg-white rounded-xl px-4 mb-3 border-2 border-amber-800 shadow-inner text-center">
                        ...
                        <div
                          ...
                          Курс: 1 PD ={" "}
                          {PDOLLAR_TO_PCOIN_RATE.toString().replace(
                              /\B(?=(\d{3})+(?!\d))/g,
                              " "
                          )}{" "}
                          PC
                        ...
                        <div className="mt-2">
                          <ActionButton
                              label={
                                bankStore.creating ? "Обмениваем..." : "Обменять"
                              }
...
function CurrencyInput({
  ...
  // Функция для обработки изменений значения
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Убираем пробелы и форматируем
  ...
  return (
      <div>
        <div className="flex justify-between items-center mb-1 px-2">
          ...
          Баланс: {balance?.toLocaleString("ru-RU")?.replace(/,/g, " ") || "0"}
        </span>
        </div>
        <div className="relative">
          <div className="bg-white rounded-xl px-4 mb-3 border-2 border-amber-800 shadow-inner text-center">
            <input
                ...
                className="bg-transparent w-full px-4 py-2 text-center font-bold text-lg sm:text-xl text-amber-800 shantell border-none outline-none"
            />
          </div>
        </div>
      </div>
