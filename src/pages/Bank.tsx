...
// Компонент модального окна для обмена
function ExchangeModal({
  ...
  // Инициализируем состояние значением, которое пришло с главного экрана (initialAmount)
  ...
  // Проверяем баланс для отображения UI
  ...
  /** Отправка заявки на ручной вывод PDollar → TON */
...
function CurrencyCard({ icon, value, label }: { icon: string; value: number; label: string }) {
  return (
      <div className="relative flex-1 max-w-[32%] drop-shadow-md transition-all">
        <img src={`${store.imgUrl}img_block.png`} alt={label} className="w-full h-12 rounded-lg" />
...
function CurrencyInput({ icon, label, value, onChange, placeholder, min }: { icon: string; label: string; balance: number; value: string; onChange: (v: string) => void; placeholder?: string; min?: number }) {
  return (
      <div>
...
function ArrowDown() {
  return (
      <div className="text-center mb-2">
        <img src={`${store.imgUrl}icon_arrow_down.png`} alt="down" className="w-6 h-auto sm:w-8 inline-block" />
      </div>
  );
}

function ActionButton({ label, onClick, disabled, img, textColor }: { label: string; onClick: () => void; disabled?: boolean; img: string; textColor?: string }) {
  return (
      <button onClick={onClick} disabled={disabled} className="relative w-full flex justify-center hover:opacity-90 transition-opacity cursor-pointer">
        <img src={img} alt={label} className="w-1/3 h-auto" />
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={`${textColor || "text-amber-800"} text-md sm:text-lg shantell`}>{label}</span>
        </div>
      </button>
  );
}
