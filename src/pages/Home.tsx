...
const Home = observer(() => {
    ...
    const [selectedSubscription, setSelectedSubscription] = useState<number | null>(null); // 7/14/30
    ...
    return (
        <>
            <div className="relative w-full min-h-screen overflow-y-auto bg-[#FFBC6B]">
                                ...
                                {floorCost}
                                  ...
                                  <span className="text-white text-sm sm:text-md shantell font-bold">
                                    {floorData.floorId} этаж
                                                            ...
                                                            {/* Звезды уровня этажа */}
                                                            <div
                                                                className="flex items-center gap-0.5 -translate-x-[20px]">
                                                            ...
                                                            {manager && (
                                                                <div
                                                                    className="flex items-center gap-1 -translate-x-[6px]">
                                                                    <img
                                                                        src={`${store.imgUrl}${
                                                                            manager.owned
                                                                                ? "Manager_icon.png"
                                    ...
                                    {manager.owned ? manager.staffLevel : 0}
                                                            ...
                                                            {guard && (
                                                                <div
                                                                    className="flex items-center gap-1 ml-2 -translate-x-[4px]">
                                                                    <img
                                                                        src={`${store.imgUrl}${
                                                                            guard.owned
                                                                                ? "Guard_icon.png"
                ...
                >
                    <div
                        className="absolute top-8 left-1/2 transform -translate-x-1/2 flex items-center justify-center text-2xl md:text-4xl text-blue-900 shantell">
                                ...
                                <div className="p-6 space-y-4 pt-8 flex flex-col items-center text-center">
                                    ...
                                    <div className="flex justify-between gap-2 mb-6 w-full">
                                        {store.userStaff?.accountantLevel?.map(
                                            (opt: { id: number; cost: number; duration?: number; durationDay?: number }) => (
                                                <button
                                                    key={opt.id}
                                                    onClick={() => {
                                                        const dur = opt.duration ?? opt.durationDay ?? null;
                                                        setSelectedSubscription(dur);
                                                        setSelectedCost(opt.cost);
                                                    }}
                                                    className={`flex-1 relative transition-opacity ${
                                                        selectedSubscription === (opt.duration ?? opt.durationDay)
        ...
        <span className="text-xs text-blue-800 font-bold">{opt.cost} pcoin</span>
      </span>
                                                </button>
                                            )
                                        )}
                                    ...
                                    {/* кнопка нанять */}
                                    <button
                                        onClick={() => selectedSubscription && handleHireStaff(selectedSubscription)}
                                        disabled={!selectedSubscription}
                                        className={`w-full relative transition-opacity ${
                                            selectedSubscription ? "hover:opacity-90" : "opacity-60 cursor-not-allowed"
                                        }`}
                                    >
                                        <img
                                            src={`${store.imgUrl}b_blue_small.png`}
                                            ...
                                            className="w-full"
                                        />
                                        <span className="absolute inset-0 flex flex-col items-center justify-center text-blue-900 shantell font-bold">
                                    <span className="text-lg">Нанять</span>
                                            {selectedCost > 0 && (
                                                <span className="text-xs text-amber-700">
                                                        за{selectedCost.toLocaleString()}pcoin
                                                </span>
                                            )}
                                    </span>
                                    </button>
