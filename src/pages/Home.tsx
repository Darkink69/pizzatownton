...
const Home = observer(() => {
  ...
  const getStaffName = (type: string | null) =>
      type === "manager" ? "Manager" : type === "accountant" ? "Accountant" : type === "guard" ? "Guard" : "";
  ...
  const handleHireStaff = (option?: number) => {
    ...
    let staffId = 0, level: number | undefined, subscription: number | undefined;
    ...
    else showNotification(`Ошибка при найме ${getStaffName(staffModal)}`, "error");
          ...
          {isModalOpen && selectedFloor && (
              ...
              <span className="text-sm sm:text-base text-amber-800 shantell">
                Текущий уровень: {selectedFloor.level}/5
                      ...
                      {(selectedFloor?.upgradeCurrency || selectedFloor?.currency) === "pcoin" && (
                          <div className="mb-6 p-4 bg-amber-50 rounded-xl border-2 border-amber-300 shadow-inner">
                            ...
                            <div className="flex flex-col gap-3">
                              {/* Охранник */}
                              <div className="flex items-center justify-between px-2 py-2 bg-white rounded-lg shadow hover:bg-amber-50 transition">
              ...
              Снижает потери дохода PDollar
                                ...
                                <button
                                    onClick={() => {
                                      ...
                                      store.sendHireStaff(staffId, 1, undefined, selectedFloor.floorId);
                                      showNotification(
                                          `👮 Охранник нанят на этаж ${selectedFloor.floorId}!`,
                                          "success"
                                      );
                                    }}
                                    className="relative bg-yellow-400 hover:bg-yellow-300 text-amber-900 px-4 py-1 rounded-lg shantell font-bold text-sm shadow"
                                >
                                  Купить/Апгрейд
                                </button>
                              </div>
              ...
              Повышает доход PDollar/час
                                ...
                                <button
                                    onClick={() => {
                                      ...
                                      store.sendHireStaff(staffId, 1, undefined, selectedFloor.floorId);
                                      showNotification(
                                          `👔 Менеджер нанят на этаж ${selectedFloor.floorId}!`,
