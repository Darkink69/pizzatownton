...
const Home = observer(() => {
  ...
  const dailyTotal = Number(store.foodStatus?.dailyTotal ?? 0);
  const hasStatus = store.foodStatus != null;
  /// если нет этажей (dailyTotal=0) — можно вообще без подсветки
  const fridgeStateClass =
      !hasStatus
  ...
  const handleUpgradeFromModal = () => {
    ...
    console.log("UPGRADE MODAL selectedFloor:", selectedFloor);
    ...
    if (success) {
      setTimeout(() => {
        console.log("UPGRADE SEND floorId=", selectedFloor.floorId, "level=", selectedFloor.level);
        showNotification(
      ...
      <>
        <div className="relative w-full min-h-screen overflow-y-auto bg-[#FFBC6B]">
          ...
          {randomNotification && (
              <div
                ...
                <div className="relative mb-4 sm:mb-20 flex items-end gap-3 sm:gap-6 max-w-4xl mx-auto px-4 w-full">
                  ...
                  <div
                      className="relative bg-[#FFF3E0] border-4 border-amber-800 rounded-2xl shadow-2xl p-4 sm:p-5 flex-1 max-w-2xl">
                    <p
                        className={`text-amber-800 shantell font-bold text-base sm:text-lg leading-relaxed whitespace-pre-wrap ${
                      ...
                      >
                        ...
                        {isFilled && floorData && !isBasementImage && !isRoof && (
                            <>
                              ...
                              <div
                                  className="absolute -top-10 left-2/5 transform -translate-x-1/2 translate-y-1/2 z-40 w-4/5 max-w-xs">
                                <div className="flex items-center relative">
                                  <img
                                  ...
                                  <div className="absolute inset-0 flex items-center">
                                    ...
                                    {manager && (
                                        <div className="flex items-center gap-1 -translate-x-[6px]">
                                          ...
                                          />
                                          <span
                                              className="text-amber-800 text-xs sm:text-sm shantell font-bold -translate-x-[3px]">
                                    ...
                                    {guard && (
                                        <div className="flex items-center gap-1 ml-2 -translate-x-[4px]">
                                          ...
                                          />
                                          <span
                                              className="text-amber-800 text-xs sm:text-sm shantell -translate-x-[3px]">
          ...
          >
            ...
            {/* Холодильник (глобальные продукты) — слева рядом с балансами */}
            <button
                type="button"
                onClick={() => {
            ...
            <div
                id="bank-link"
                className="absolute -bottom-5 left-6 flex flex-col justify-between items-center"
            >
          ...
          >
            <div
                className="absolute top-1 left-1/2 transform -translate-x-1/2 flex items-center justify-center text-sm md:text-xl text-blue-900 shantell">
              ...
              Холодильник
                ...
                {toNum(store.foodStatus?.dailyTotal).toLocaleString()}
                  ...
                  {toNum(store.foodStatus?.weeklyPrice).toLocaleString()}
                      ...
                      <button
                          disabled={!canBuyFood || isBuyingFood}
                          onClick={() => {
                            if (!canBuyFood || isBuyingFood) return;

                            playSound("buy.mp3");

                            const ok = store.startFoodBuy();
                            if (!ok) {
                              showNotification("Не удалось отправить запрос на покупку продуктов", "error");
                            } else {
                              showNotification("Покупаем продукты на 7 дней...", "success");
                            }
                          }}
                          className={`w-full relative py-2 rounded-lg flex items-center justify-between px-4 
                          transition-opacity ${(!canBuyFood || isBuyingFood) ? "opacity-50 cursor-not-allowed" : "hover:opacity-90"}`}
                      >
                        <img
                        ...
                        {[1, 2, 3, 4, 5].map((star, index) => {
                          ...
                          return (
                              <div key={star} className="flex flex-col items-center">
                                ...
                                />
                                <div
                                    className="flex items-center gap-0 bg-white px-2 py-1 rounded border border-amber-800">
                                ...
                                (opt: {
                                  ...
                                  durationDay?: number;
                                    ...
                                    >
                                      ...
                                      />
                                      <span
                                          className="absolute inset-0 flex flex-col items-center justify-center text-amber-800 shantell text-sm">
                              <span>
                  ...
                  {jettonUiMessage && jettonUiType === "success" && (
                      ...
                      <span className="text-green-800 font-bold shantell text-xl">
                        ...
                        >
                          ...
                          />
                          <span
                              className="relative z-10 px-10 text-center text-white font-bold shantell leading-none whitespace-nowrap text-[16px]">
